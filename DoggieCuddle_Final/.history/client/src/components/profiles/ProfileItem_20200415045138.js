...
const ProfileItem = (props) => {
  ...
  return (
     ...
     <GridListTile key={d._id} cols={d.dogName ? 2 : 1} rows={d.dogName ? 2 : 1}>
     <img src={d.image}  />
     <GridListTileBar
              title={d.dogName}
              titlePosition="top"
              subtitle={<span> {d.location}</span>}
              subtitle={<span> {d.breed}</span>}
              actionIcon={
                <IconButton aria-label={`star ${d.dogName}`} className={classes.icon}>
                  <StarBorderIcon />
